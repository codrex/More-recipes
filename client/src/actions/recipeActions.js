import { RECIPE, UPDATE_DIRECTIONS,
          ...
          TOGGLE_FAV, RECIPE_TO_MODIFY,
          CURRENT_RECIPE, DELETE_RECIPE, ON_NEW_RECIPE } from './actions';
...
export const createRecipe = (recipe, msg) => (dispatch) => {
  ...
  dispatcher.requestAndDispatch('/api/v1/recipes',
                  recipe, createOrModifyRecipe, 'post', msg);
};

export const modifyRecipe = (recipe, msg) => (dispatch) => {
  const dispatcher = new ActionDispatcher(dispatch);
  dispatcher
    .requestAndDispatch(`/api/v1/recipes/${recipe.id}`,
                recipe, createOrModifyRecipe, 'put', msg);
};
...
export const toggleFav = (recipeId, msg = undefined) => (dispatch) => {
  const dispatcher = new ActionDispatcher(dispatch);
  const id = dispatcher.getIdFromToken();
  dispatcher.requestAndDispatch(`/api/v1/users/${id}/recipe`,
                                  recipeId, afterToggleFav, 'post', msg);
};
